...
export const DashboardPropertyCard = ({
  ...
  const handleAddToSignature = async () => {
    try {
      ...
      const currentSignatureResponse = await fetch(
        `${process.env.NEXT_PUBLIC_MYSQL_ENDPOINT}/get-property-by-id/${id}`,
        {
          method: "GET",
        }
      );
      ...
      const updateSignatureResponse = await fetch(
        `${process.env.NEXT_PUBLIC_MYSQL_ENDPOINT}/set-signature-property/${id}`,
        {
      ...
      const result = await updateSignatureResponse.json();
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const deleteDescriptionResponse = await fetch(
        `${process.env.NEXT_PUBLIC_MYSQL_ENDPOINT}/delete-property-description-by-property-id/${id}`,
        {
          method: "DELETE",
        }
      );
      ...
      const deletePropertyResponse = await fetch(
        `${process.env.NEXT_PUBLIC_MYSQL_ENDPOINT}/delete-property-by-id/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    ...
    >
      ...
      <div
        className={
          "w-full flex items-center justify-end p-4 absolute top-0 right-0 z-10"
        }
      >
        {/* <Popover>
          <PopoverTrigger asChild>
            <div
              className={
                "rounded-full p-1 border border-black bg-coolGray duration-300 w-6 h-6 flex items-center justify-center"
              }
            >
